...
def extract_api_key(API_KEY_PATH):
  ...
  #  print(f"An error occurred while connecting to Mistral: {e}")
  #  return None
  return api_key
...
API_KEY_PATH = "./api_key_mistral.txt"
...
last_index_component = 0#len(COMPONENTS_DICT) - 1
number_iteration = 3
...
for i in range(first_index_component, last_index_component + 1):
  ...
  metadata = create_metadata(API_KEY_PATH, AGENT1_PATH, MODEL, f"../Components/{component_keys[i]}", f"../Results/Benchmark/{component_keys[i]}", component_values[i])
  for j in range(3, number_iteration + 1): 
    print(f"Iteration {j}")
    create_code(API_KEY_PATH, AGENT2_PATH, AGENT3_PATH, MODEL, f"../Components/{component_keys[i]}", f"../Results/Benchmark/{component_keys[i]}{j}/", component_values[i], metadata)
  concatenate_JSON(API_KEY_PATH, AGENT4_PATH, MODEL, f"../Results/Benchmark/{component_keys[i]}/", f"../Components/{component_keys[i]}", f"../Results/Benchmark/{component_keys[i]}/", component_values[i], metadata, number_iteration)
...
# LINTUL section
for i in range(0, len(LINTUL_FILES)):
...
for i in range(0, len(cross_st_keys)):
  print(f"Processing {cross_st_keys[i]}")
  for j in range(0, len(cross_mp_keys)):
