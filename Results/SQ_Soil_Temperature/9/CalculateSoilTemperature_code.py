...
def Estimate(
    ...
    """
    Main biophysical process: calculate soil minimum and maximum temperature,
    then update deep layer soil temperature.

    Inputs:
    - deeplayerstates_deepLayerT: float (°C), current deep soil layer temperature
    - exogenous_maxTAir: float (°C), daily maximum air temperature
    - exogenous_meanTAir: float (°C), daily mean air temperature
    - exogenous_minTAir: float (°C), daily minimum air temperature
    - ratesexternal_heatFlux: float (g m-2 d-1), soil heat flux (mass basis)
    - lambda_: float (MJ kg-1), latent heat of vaporization at 20°C (default 2.454)

    Returns:
    - states_minTSoil: float (°C), daily minimum soil temperature
    - states_maxTSoil: float (°C), daily maximum soil temperature
...
def UpdateTemperature(minSoilTemp: float, maxSoilTemp: float, Temperature: float) -> float:
    """
    Update deep soil layer temperature (°C) using an exponential smoothing of daily mean.
