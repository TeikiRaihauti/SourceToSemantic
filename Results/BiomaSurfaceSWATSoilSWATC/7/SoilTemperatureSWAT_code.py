def Init(states_LayerThickness):
    ...
    Inputs:
    - states_LayerThickness: list or sequence of soil layer thicknesses (m)

    Returns:
    - states_SoilTemperatureByLayers: list of initial soil temperatures by layer (degC), initialized to 15 for each layer
    """
...
def Estimate(LagCoefficient,
    ...
    Inputs:
    - LagCoefficient: float, lag coefficient controlling influence of previous day's temperature (dimensionless, 0..1)
    ...
    - states_SoilTemperatureByLayers: list of floats, current soil temperature of each layer (degC)
    ...
    - states_LayerThickness: list of floats, thickness of each soil layer (m)
    ...
    updated_states_SoilTemperatureByLayers[0] = (
        LagCoefficient * updated_states_SoilTemperatureByLayers[0]
        + (1.0 - LagCoefficient)
        * (
    ...
    for i in range(1, len(states_LayerThickness)):
        ...
        updated_states_SoilTemperatureByLayers[i] = (
            LagCoefficient * updated_states_SoilTemperatureByLayers[i]
            + (1.0 - LagCoefficient)
            * (
                _DepthFactor * (exogenous_AirTemperatureAnnualAverage - states_SurfaceSoilTemperature)
                + states_SurfaceSoilTemperature
