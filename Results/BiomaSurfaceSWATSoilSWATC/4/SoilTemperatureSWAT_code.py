...
def Init(states_LayerThickness):
    ...
    Inputs:
    - states_LayerThickness: list of float, soil layer thickness (m) for each layer

    Returns:
    - states_SoilTemperatureByLayers: list of float, initialized soil temperature of each layer (degC)
    """
    states_SoilTemperatureByLayers = [15.0] * len(states_LayerThickness)
...
def Estimate(LagCoefficient,
    ...
    - states_VolumetricWaterContent: list of float, volumetric soil water content (m3 m-3) per layer
    - states_LayerThickness: list of float, soil layer thickness (m) per layer
    - states_BulkDensity: list of float, bulk density (t m-3) per layer
    - states_SoilTemperatureByLayers: list of float, previous day's soil temperature (degC) per layer

    Returns:
    - states_SoilTemperatureByLayers: list of float, updated soil temperature (degC) per layer
    """
    ...
    new_SoilTemperatureByLayers = [0.0] * n_layers
